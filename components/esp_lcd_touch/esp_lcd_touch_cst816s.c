...
// Task 4 (M2.5, knobos): vendored verbatim from espressif/esp-bsp
// (esp_lcd_touch_cst816s v1.1.2) — see esp_lcd_touch.c in this component for
// full provenance/rationale. CST820 (VIEWE UEDX46460015-MD50E's actual touch
// chip, per its datasheet in the /tmp/viewe-s3 clone) is protocol-compatible
// with CST816S — same register map (DATA_START_REG=0x02 num/x/y, CHIP_ID_REG
// 0xA7), confirmed by diffing this file against the dedicated CST820 port in
// that clone (Libraries/ESP32_Display_Panel/src/drivers/touch/port/
// esp_lcd_touch_cst820.c): that port reads 15 bytes from reg 0x00 instead of
// 6 bytes from reg 0x02, but slices out num/x_h/x_l/y_h/y_l at the same
// register offsets (0x02..0x06) — same data, different read framing. Not
// modified beyond this comment.

...
// Task 4 (M2.5, knobos): vendored verbatim from the official Espressif esp-bsp
// repo, https://github.com/espressif/esp-bsp/tree/master/components/lcd_touch/esp_lcd_touch
// (esp_lcd_touch v1.2.1, commit faede148a50a035ba0c29c578e24250b4f2362e8,
// fetched 2026-09-17), fetched via GitHub raw — the component registry
// (components.espressif.com/components-file.espressif.com) was unreachable
// from this machine at implementation time, same failure as Task 2's co5300
// driver. Unlike Task 2, no framework-dependency stripping was needed here:
// this component is already standalone (no ESP32_Display_Panel coupling),
// which is also why it (rather than the older VIEWE-fork copy under
// /tmp/viewe-s3) was chosen — esp_lvgl_port 2.9.0 (already resolved via the
// component registry into managed_components before it went down) expects
// this newer esp_lcd_touch API (esp_lcd_touch_get_data/esp_lcd_touch_point_data_t);
// the older fork's esp_lcd_touch v1.1.2 API predates that and doesn't link
// against esp_lvgl_port_touch.c. Not modified beyond this comment.
